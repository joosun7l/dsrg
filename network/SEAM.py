...
from tool import pyutils, visualization
from torchvision.utils import save_image
...
class Net(network.resnet38d.Net):
    ...
    def forward(self, x):
        ...
        cam_d_norm = self.cls_corr(cam_d_norm)
        cam_rv = self.cls_corr(cam_rv)
        
        # cam_rv_ = F.interpolate(cam_rv, (H,W), mode='bilinear', align_corners=True)
        # save_image(cam_rv_,'test2.png')
        # cam_d_norm_ = F.interpolate(cam_d_norm, (H,W), mode='bilinear', align_corners=True)
        return cam_rv,cam_d_norm
        ...
        for i in range(cam_shape[1]):
            for j in range(cam_shape[0]):
                if i == 0:
                    ...
                    cam[j,i][cam[j,i] <= 0.1] = 0
                    cam[j,i][cam[j,i] <= cam[j,i].max()*0.25] = 0
